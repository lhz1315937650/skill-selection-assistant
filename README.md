...
Before each recommendation, the router performs a lightweight freshness check against the manifest: it compares local `SKILL.md` paths, sizes, and modification times without reading every file body. If the installing user added, removed, or changed a skill, the router rebuilds the local classification index before recommending anything. This keeps recommendations tied to skills that are actually installed on that machine.
