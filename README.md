...
Run this command from the installed router skill directory when you explicitly want to refresh the deep index. Unchanged classifications are reused by default; add `--full-rebuild` only for a complete reclassification:

```powershell
python scripts/deep-classify-skills.py --leaf-target 24
python scripts/deep-classify-skills.py --leaf-target 24 --full-rebuild
```
...
The generated `.skill-index/deep/` directory belongs only to the installing user. It contains the detailed catalog, structured function profiles, multi-label facets (including multiple setup requirements), compact route cards, classification evidence, and `source-manifest.json` freshness data for every discovered source, including classification failures. It must never be committed or shipped in a release. Normal routing exposes only dynamically selected current branches or a small final shortlist.
...
Before each recommendation, the router performs a lightweight freshness check against the manifest: it compares local `SKILL.md` paths, sizes, and modification times without reading every file body. If the installing user added, removed, or changed a skill, the router incrementally refreshes the index and reclassifies only changed sources when the schema, classifier, and rules remain compatible.

`DETAILED_CLASSIFICATION.md` is generated by `scripts/summarize-index.py`. The Python and PowerShell installers try to create it automatically after a successful first scan, and `doctor.ps1 -Fix` can regenerate it when missing.
...
The preferred cross-platform recommender returns the stable schema `3.0.0` envelope (`mode`, `route`, `branches`, and `candidates`):

```bash
python skill-selection-assistant/scripts/recommend-skills.py --query "build a frontend UI"
```

PowerShell users can use the equivalent wrapper or its legacy compatibility commands:
