...
def agents_block(destination: Path) -> str:
    return "\n".join([
        ...
        "3. Never show intermediate categories or ask the user to classify the request; present only the final returned weighted skills. For every skill, include its name, a concise one-sentence summary of what it does (from function_summary), and its weight or match reason; never output a bare skill name before asking which one to use.",
